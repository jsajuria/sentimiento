#' create_matrix
#'
#' Creates an object of class \code{DocumentTermMatrix} from \pkg{tm}.The default language has been changed to Spanish.
...
#' @param removeSpareTerms See package \pkg{tm} for more details.
...
#' @param qeighting Either \code{weightTf} or \code{weightTfIdf}. See package \pkg{tm} for more details.
#' @keywords sentiment
#' @export
#' @examples
#' # LOAD LIBRARY
...
#' @author Timothy P. Jurka \email{tpjurka@@ucdavis.edu}

create_matrix <- function(textColumns, language="spanish", minDocFreq=1, minWordLength=3, removeNumbers=TRUE, removePunctuation=TRUE, removeSparseTerms=0, removeStopwords=TRUE, stemWords=FALSE, stripWhitespace=TRUE, toLower=TRUE, weighting=weightTf) {
